...
class SAPayload(Payload):
	...
	def __init__(self, proposals: list["Proposal"]):
		super().__init__(consts.IKEV2_PAYLOAD_SA, proposals)

	@classmethod
	def parse(cls, buf: bytes, id: bytes):
		res = cls(Payload.parse(buf, consts.IKEV2_SUB_PROPOSAL, id))
		return res
...
class Proposal(Payload):
	...
	def __init__(self, num: int, protocol: int, spi: bytes, transforms: list["Transform"]):
		...
		self.spi = spi
		super().__init__(consts.IKEV2_SUB_PROPOSAL, transforms)
	...
	def parse(cls, buf: bytes, id: bytes):
		...
		spi = buf[4:4 + spilen]
		return cls(num, protocol, spi, Payload.parse(buf[4 + spilen:], consts.IKEV2_SUB_TRANSFORM, id))
...
class Transform(Payload):
	...
	def __init__(self, transtype: int, id: int, attributes: list["Attribute"]):
		...
		self.id = id
		super().__init__(consts.IKEV2_SUB_TRANSFORM, attributes)
...
class KEPayload(Payload):
	...
	def __init__(self, dh_group: int, data: bytes):
		...
		self.data = data
		super().__init__(consts.IKEV2_PAYLOAD_KE, [Raw(data)])
...
class NoncePayload(Payload):
	def __init__(self, data: bytes):
		self.data = data
		super().__init__(consts.IKEV2_PAYLOAD_NONCE, [Raw(data)])
...
class NotifyPayload(Payload):
	def __init__(self, type: int, msg: bytes):
		...
		super().__init__(consts.IKEV2_PAYLOAD_NOTIFY, [Raw(buf)])
...
class EncryptedPayload(Payload):
	def __init__(self, id: bytes, children: list["Payload"]):
		self.id = id
		super().__init__(consts.IKEV2_PAYLOAD_ENCRYPTED, children)
...
class IdentityPayload(Payload):
	...
	def parse(cls, buf: bytes, id: bytes):
		...
		data = buf[4:]
		return cls(consts.IKEV2_PAYLOAD_IDI, idtype, data)
...
class AuthPayload(Payload):
	def __init__(self, method: int, data: bytes):
		self.method = method
		self.data = data
		super().__init__(consts.IKEV2_PAYLOAD_AUTH, [Raw(data)])
...
class DeletePayload(Payload):
	def __init__(self, protocol: int, spis: list[bytes]):
		self.protocol = protocol
		self.spis = spis
		super().__init__(consts.IKEV2_PAYLOAD_DELETE, [])
...
payload_map = {
	consts.IKEV2_PAYLOAD_SA: SAPayload,
	consts.IKEV2_SUB_PROPOSAL: Proposal,
	consts.IKEV2_SUB_TRANSFORM: Transform,
	consts.IKEV2_PAYLOAD_KE: KEPayload,
	consts.IKEV2_PAYLOAD_NONCE: NoncePayload,
	consts.IKEV2_PAYLOAD_NOTIFY: NotifyPayload,
	consts.IKEV2_PAYLOAD_ENCRYPTED: EncryptedPayload,
	consts.IKEV2_PAYLOAD_IDI: IdentityPayload,
	consts.IKEV2_PAYLOAD_IDR: IdentityPayload,
	consts.IKEV2_PAYLOAD_AUTH: AuthPayload,
	consts.IKEV2_PAYLOAD_DELETE: DeletePayload,
}
...
class Message:
	...
	def build(self):
		...
		if self.children[-1].type == consts.IKEV2_PAYLOAD_ENCRYPTED:
		...
		try:
			...
			if not encrypted:
				...
				while i < length:
					if buf[i] == 0:
						break
					elif buf[i] == consts.IKEV2_PAYLOAD_ENCRYPTED:
						encrypted = True
						break
