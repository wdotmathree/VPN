...
thing: typing.Dict[bytes, typing.List[bytes]] = {}

def recv(id):
	while len(thing[id]) == 0:
		sleep(0.001)
	return thing[id].pop(0)
...
while True:
	...
	if sid == 0:
		...
		thing[id] = [buf]
		handle(id)
		# Thread(target=handle, args=(a, str(id)), daemon=True).start()
		break
	else:
